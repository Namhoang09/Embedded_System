...
void TIM3_Init(void) {
    RCC->APB1ENR |= (1 << 1);         // Bật clock TIM3

    TIM3->PSC = 16000 - 1;            // 16MHz / 16000 = 1kHz
    TIM3->ARR = 500 - 1;                  // 500ms
    TIM3->CNT = 0;

    TIM3->DIER |= (1 << 0);           // Cho phép ngắt update
    TIM3->CR1 |= (1 << 0);            // Bật TIM3

    NVIC_EnableIRQ(TIM3_IRQn);        // Cho phép TIM3 interrupt trong NVIC
}

void TIM3_IRQHandler(void) {
    if (TIM3->SR & (1 << 0)) {
        TIM3->SR &= ~(1 << 0);        // Xóa cờ update
        ADC1_Start();                 // Gọi chuyển đổi ADC
    }
}
